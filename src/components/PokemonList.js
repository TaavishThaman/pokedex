...
function PokemonList() {
  ...
  //   const fetchPokemonDetailedData = useCallback(
  //     () =>
  //       ,
  //     [pokemonBaseData]
  //   );
  ...
  function filterList(term) {
    setFilteredPokemonData(
      pokemonDetailedData.filter((e) =>
        e["data"]["name"].includes(term.toLowerCase())
      )
    );
  }
  ...
  function applyFilters() {
    if (activeFilters["type"].length > 0) {
      ...
      if (types.length === 1) {
        ...
        setFilteredPokemonData(
          pokemonDetailedData.filter((ele, idx) => arr.includes(idx))
        );
      } else {
        pokemonDetailedData.map((ele, idx) => {
  ...
  async function loadMorePokemonBaseData(baseData) {
    let promiseData = await Promise.all(
      baseData.map((d) => fetchAndProcessData(d.url))
    );
  ...
  useEffect(() => {
    async function fetchPokemonDetailedData() {
      let promiseData = await Promise.all(
        pokemonBaseData.map((d) => fetchAndProcessData(d.url))
      );
  ...
  return (
    <div style={listStyle}>
                      ...
                      style={
                        Object.keys(typeBackgrounds).includes(ele)
                          ? Object.assign(
                              ...
                              typeStyle,
                              typeBackgrounds[ele]
                                ? { backgroundColor: typeBackgrounds[ele] }
                                : {}
                            )
                          : typeStyle
                      }
                  ...
                  return (
                    <div
                      ...
                      style={Object.assign(
                        {},
                        typeStyle,
                        typeBackgrounds[ele.name]
                          ? { backgroundColor: typeBackgrounds[ele.name] }
                          : {}
                      )}
                    >
                      {
      ...
      {!loading && pokemonDetailedData.length > 0 ? (
        <div style={inputDiv}>
      ...
      {loading && pokemonDetailedData.length === 0 ? (
        ...
        <div style={cardContainer}>
          {filteredPokemonData.map((ele) => {
            return (
              <PokemonCard
                key={String(ele["data"]["id"])}
                id={String(ele["data"]["id"])}
                name={ele["data"]["name"]}
                image={
                  ele["data"]["sprites"]["other"]["official-artwork"][
                    "front_default"
                  ]
                }
